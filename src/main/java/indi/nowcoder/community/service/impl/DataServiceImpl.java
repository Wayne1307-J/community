...
public class DataServiceImpl implements DataService {
    ...
    public void recordUV(String ip) {
        ...
        redisTemplate.opsForHyperLogLog().add(redisKey, ip);  // 添加
    }
    // 统计指定日期范围内的UV
    @Override
    public long calculateUV(Date start, Date end) {
        ...
        while (!calendar.getTime().after(end)) { // 时间不晚于end
            String key = RedisKeyUtil.getUVKey(df.format(calendar.getTime()));
            keyList.add(key);
            calendar.add(Calendar.DATE, 1); // +1天，继续循环
        }
        // 合并这些数据
    ...
    public void recordDAU(int userId) {
        ...
        redisTemplate.opsForValue().setBit(redisKey, userId, true);  // 调用表明就活跃了，设置为true
    }
